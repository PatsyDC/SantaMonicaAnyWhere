...
class TrabajosRealizadosDetailUpdateDeleteAPIView(APIView):
    serializer_class = TrabajosRealizadosSerializer
    ...
    def get(self, request, dni, *args, **kwargs):
        ...
        if not trabajador:
            return Response({"error": f"No se encontró ningún trabajador con el DNI {dni}"},
                            status=status.HTTP_404_NOT_FOUND)

        # Obtener el registro de TrabajosRealizados por el DNI del trabajador y la fecha actual
        trabajos_realizados = TrabajosRealizados.objects.filter(trabajador_dni=trabajador, fecha=fecha_actual)

        serializer = self.serializer_class(trabajos_realizados, many=True)
        return Response(serializer.data)

    def put(self, request, dni, *args, **kwargs):
        # Obtener la fecha actual
        fecha_actual = date.today()
        ...
        if not trabajos_realizados.exists():
            return Response({"error": "No se encontraron registros para actualizar en la fecha actual."},
                            status=status.HTTP_404_NOT_FOUND)

        # Actualizar el campo 'kg' del registro
        instance = trabajos_realizados.first()
        serializer = TrabajosRealizadosSerializer(instance, data={'kg': request.data.get('kg')}, partial=True)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
from .serializers import IngresoTrabajadoresSerializer, TrabajosRealizadosSerializer, SalidaTrabajadoresSerializer
from datetime import date
...
class TrabajadoresIngresadosHoyAPIView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        for ingreso in ingresos_produccion:
            trabajos_ingreso = TrabajosRealizados.objects.filter(trabajador_dni__dni=ingreso.trabajador.dni, fecha=fecha_actual)
